...
class Analyzer:
    ...
    def __init__(self, recording_file, bin_size):
        ...
        self.t_start = datetime.datetime.fromisoformat(self.file.readline()[12:-1]).replace(microsecond=0)
    ...
    def analyze(self):
        ...
        while True:
            ...
            if not line:
                print(f"Processed bin {len(n_msg_bins)}: n_msg={n_msg_bins[-1]}   msg/s={n_msg_bins[-1]/self.bin_size:.1f}")
                break
            ...
            if offsets[-1] // self.bin_size > len(n_msg_bins)-1:
                if len(offsets) > 1 and offsets[-2] // self.bin_size == len(n_msg_bins)-1:
                    print(f"Processed bin {len(n_msg_bins)}: n_msg={n_msg_bins[-1]}   msg/s={n_msg_bins[-1]/self.bin_size:.1f}")
                n_msg_bins.append(0)
        ...
        print(f"Start: {str(self.t_start + datetime.timedelta(seconds=round(offsets[-1])))}")
